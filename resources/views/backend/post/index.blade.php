...
@push('prescripts')
<script>
	var datatableRoute = '{!! isset($category->id) ? route('backend.post.data.category', $category->id) : route('backend.post.data') !!}';
...
<div class="btn-group">
    ...
    <ul class="dropdown-menu pull-left">
        <li><a href="{!!route('backend.post.index')!!}">Tất cả</a></li>
        @foreach ($rootCategories as $rootCategory)
        <li><a href="{{route('backend.post.category', $rootCategory->id)}}">{{$rootCategory->name}}</a></li>
        @endforeach
    </ul>
</div>
@endpush
